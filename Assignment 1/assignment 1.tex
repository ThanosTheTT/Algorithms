...
\end{center}
ΑΚ$\neq$XΠ
Αρα κανένας υποψήφιος του πίνακα Α δεν έχει πλειοψηφία πάνω απο 50\%\\
O πίνακας Π θα έχει και επανάλυψη \textcolor{red}{i=6}
γιατί χρησιμοποιούμε πάνω όριο στον βρόχο επανάλυψης ($\lceil\frac{n}{2}\rceil$):
\begin{center}
...
\end{center}
\textcolor{red}{XΠ=ΧΠ}
Aρα στο παράδειγμα μας ο ΧΠ έχει πλειοψηφία πάνω απο 50\%.
    ...
    \item Aνακεφαλαιώνοντας ο αλγόριθμος αυτός σαφώς εμφανίζεται ποιο βελτιωμένος από τον πρώτο καθώς έχουμε ελλατώσει τις συγκρίσεις σε μεγάλο βαθμό με την βοήθεια του devide and conquer αλλά και πάλι εμφανίζονται αρκετές επαναλήψεις ανάγνωσης ψήφου .Συνολικά η πολυπλοκότητα είναι $\mathcal{O}(nlogn+n)=\mathcal{O}(nlogn)$
    \end{itemize}
...
\end{itemize}
Επειδή τόσο οι εντολές T.search και Τ.put για ένα hashmap τρέχουν σε O(1)\footnotemark[7] και χρησιμοποιούμε ένα βρόγχο που επαναλαμβάνεται (n) φορές, πετυχαίνουμε συνολικό χρόνο O(n).\footnotetext[7]{Για να ισχύει ότι η αναζήτηση και η τοποθέτηση του ονόματος(string) του υποψηφίου τρέχει σε χρόνο Ο(1) πρέπει να αποφεύγονται οι συγκρούσεις. Ο καλύτερος τρόπος για να αποφεύγονται οι συγκρούσεις είναι χρησιμοποιώντας μια καλή hash function όταν  η οποία κατανέμει τα ονόματα των υποψηφίων ομοιόμορφα στο hashmap.Σε περίπτωση συγκρούσεων θα γίνεται η διαδικασία του κατακερματισμού.}
